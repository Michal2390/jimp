	...
	for (j = 0; j < nb; j++) {
		for (i = 0; i < 5; i++)
			for (k = 0; k < pts->n; k++) {
				...
				for (int r = i+j; r>0; r--)
					temp *= x[k];
				add_to_entry_matrix(eqs, j, i, temp);
			}
		for (k = 0; k < pts->n; k++) {
			double temp = 1.0;
			for (int r = j; r > 0; r--)
				temp *= x[k];
			add_to_entry_matrix(eqs, j, 5, y[k] * temp );
		}
		printf("Przejscie petli j = %d\n", j);
	}
...
if (alloc_spl(spl, 5) == 0) {
			spl->f[0] = get_entry_matrix(eqs, 0, 5);	//x to a0
			spl->f[1] = get_entry_matrix(eqs, 1, 5);   // f to a1
			spl->f[2] = get_entry_matrix(eqs, 2, 5);  // f1 to a2
			spl->f[3] = get_entry_matrix(eqs, 3, 5);  // f2 to a3
			spl->f[4] = get_entry_matrix(eqs, 4, 5);  // f3 to a4
		}
/*
